...
usage:
  python place_pictograms.py 図面.pptx -o 図面_icon.pptx
...
import textfit
from pick_pictograms import load_catalog, pick_line, LIFE_KEYS, PICTO
...
def find_body(slide, head_text):
    ...
    for head in heads:
        ...
        for sh in slide.shapes:
            ...
            # 見出しのすぐ下。point_fix で本文が少し持ち上がることがあるので上側にも余裕を見る
            if t < hb - 0.4 or t > hb + 1.2:
                continue
...
def widen_life_names(slide, name_box, log):
    ...
    need = max(textfit.cm(p.text, para_pt(p)) for p in dist.text_frame.paragraphs
               if p.text.strip())
    free = cm_of(dist.width) - ins["lIns"] - ins["rIns"] - need * 1.12 - 0.25   # 書体差（数字や m の幅）で折り返さないよう余裕を残す
    if free <= 0.05:
        return 0.0, dist
