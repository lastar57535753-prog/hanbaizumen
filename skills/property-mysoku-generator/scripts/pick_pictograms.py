...
def _pictogram_dir():
    ...
    スキル同梱（assets/pictograms）を優先し、無ければリポジトリ直下の pictograms/ を使う。
    スキルを単体で配ったときも、リポジトリで開発しているときも同じコードで動くように。"""
    for cand in (os.path.join(ROOT, "assets", "pictograms"),
                 os.path.join(os.path.dirname(ROOT), "pictograms"),
                 os.path.join(ROOT, "pictograms")):
        if os.path.exists(os.path.join(cand, "catalog.json")):
            return cand
    raise SystemExit("!! ピクトグラムが見つかりません（assets/pictograms/catalog.json）")


PICTO = _pictogram_dir()
CATALOG = os.path.join(PICTO, "catalog.json")
...
def main():
    ...
    def png(key):
        return os.path.join(PICTO, "png", args.color, key + ".png")
