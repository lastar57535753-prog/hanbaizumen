...
- PDFを省くなら末尾に `--no-pdf`。
- PDF書き出しの直後に **文字の重なり検査** が走る。LibreOffice は書体と文字の
  組み合わせによっては直前の文字の送り幅を取りこぼし、次の文字を前に戻して
  描くことがある（実例:「土地面積」が「地面積」に見える／「京急ストア平和島店」が
  「京急ス平和島店」に見える）。字は正しく入っているので目視では見落としやすい。
  出たらその文言を言い換える。土地テンプレのラベルを「敷地面積」にしているのは
  この回避のため。
...
**割り当てなかった写真枠も止める。** テンプレの写真枠に何も入れないと、見本として
入っている**別物件の写真がそのまま残る**（いちばん重い事故）。使わない枠は
`"hide_ids": [枠のid]` で消す。`--allow-missing` のときは見本写真を削除して枠を空にする。
...
- キャッチは**間取図に潜り込む**と文字が絵の下に隠れる。preflight が z順で見て止めるので、
  出たら CATCH1/2 を短くする（間取図の左端まで＝おおむね全角22字）。
