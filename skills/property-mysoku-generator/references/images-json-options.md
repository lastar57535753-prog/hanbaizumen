...
- `set_font`: `[{"id":281,"size_pt":9.0}]` その箱だけフォント縮小。`"line_spacing":1.25` で行間も変えられる（POINTを増やして行数を稼ぎたいとき。既定は1.5）。
...
- `text_color`: `[{"ids":[25],"color":"14161B"}]` 文字色。**明るい写真の上に白いキャッチを置くと消える**ので、その場合は墨（14161B）＋白フチにする。暗い写真なら白文字＋黒フチのまま。
- `text_outline`: `[{"ids":[25],"color":"14161B","pt":1.1,"alpha":100}]` 袋文字。白いキャッチが明るい写真や白地に溶けるときに使う（スクリムは恒久ルールで禁止なので、輪郭線＋影で視認性を確保する）。
- `relabels`: `{"307":{"5,0":"バルコニー面積"}}` テーブルのラベル差し替え（"行,列"）。
- `text_replace`: `{"322":{"媒介":"専属専任"}, "281":{"徒歩10分":"バス10分"}}` 固定文言の差し替え（run単位・書式維持）。取引態様(id322)やバス便のアクセス行に使う。
